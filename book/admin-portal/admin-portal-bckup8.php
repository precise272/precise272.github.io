...
// Function to fetch all book data from the server
function fetchAllBookData(bookId) {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: '/book/admin-portal/books/fetch-book-data.php',
            method: 'GET',
            data: { book_id: bookId },
            dataType: 'json',
...
function displayBookDetails(bookDetails) {
    ...
    if (bookDetails !== null && bookDetails !== undefined) {
        ...
        $.each(bookDetails.book_details, function (key, value) {
            ...
            $('#bookDetailsBookId').val(bookDetails.book_details.book_id);
        });
    } else {
...
// Function to display book preferences in the editable table
function displayBookPreferences(bookPreferences) {
    // Check if book preferences are not null or undefined
    if (bookPreferences !== null && bookPreferences !== undefined) {
        // Iterate through book preferences and append columns to the row
        $.each(bookPreferences.book_preferences, function (key, value) {
            // Exclude the book_id field from being displayed
            if (key !== 'book_id') {
                // Create input fields for editing
                var inputField;
            ...
            // Update the hidden input field with the book ID
            $('#bookPreferencesBookId').val(bookPreferences.book_preferences.book_id);
        });
    } else {
...
// Revised function to load all book data at once
function loadAllBookData(bookId) {
    fetchAllBookData(bookId)
        .then((bookData) => {
            ...
            const bookDetails = bookData.book_details;
            const bookPreferences = bookData.book_preferences;
            const bookChapters = bookData.book_chapters;
...
const bookData = [
    { chapter_id: '5', book_id: '5', chapter_number: '1', storyline: null, affinity_requirement: 'neutral' },
...
async function displayPagesInAccordion(chapterId, pages) {
    var pagesList = $('#pagesAndChoicesList' + chapterId);
    pagesList.empty();

    if (pages && pages.length > 0) {
        // Iterate through pages and append each page to the list
        for (const page of pages) {
            ...
            await loadChoicesForPage(page.book_id, chapterId, page.page_id, function (choices, pageId, chapterId) {
...
function addChapterDetailsCollapse(chapter) {
    ...
    var collapseDiv = '<div class="collapse" id="chapterCollapse_' + chapter.chapter_id + '">' +
        ...
        'Chapter Number: ' + chapter.chapter_number + '<br>' +
        'Chapter Title: ' + chapter.chapter_title + '<br>' +
...
function displayPagesAndChoicesInAccordion(chapterId, pagesAndChoices) {
    // Clear existing pages and choices list
    $('#pagesAndChoicesList' + chapterId).empty();

    // Check if pages are available for the chapter
    if (pagesAndChoices.pages && pagesAndChoices.pages.length > 0) {
        // Iterate through pages and choices and append to the list
        for (let i = 0; i < pagesAndChoices.pages.length; i++) {
            const page = pagesAndChoices.pages[i];
            const choices = page.choices;

            // Append page content to the list
            $('#pagesAndChoicesList' + chapterId).append('<li class="list-group-item">Page ' + page.page_number + ': ' + page.content + '</li>');

            // Check if choices are available for the page
            if (choices && choices.length > 0) {
                // Iterate through choices and append to the list
                for (let j = 0; j < choices.length; j++) {
                    const choice = choices[j];
                    $('#pagesAndChoicesList' + chapterId).append('<li class="list-group-item">Choice ' + choice.choice_id + ': ' + choice.choice_text + '</li>');
                }
            }
        }
    } else {
        // Display a message if no pages are available for the chapter
        $('#pagesAndChoicesList' + chapterId).append('<li class="list-group-item">No pages available for this chapter.</li>');
    }
}
...
async function fetchPages(bookId, chapterId) {
    try {
        // Make an AJAX request to fetch pages based on the provided book and chapter IDs
        const response = await $.ajax({
            url: '/book/admin-portal/books/fetch-selected-book-pages.php', // Replace with your actual API endpoint
            method: 'GET',
            data: {
                bookId: bookId,
                chapterId: chapterId,
        ...
        return response.pages; // Assuming your API response has a 'pages' property
    } catch (error) {
        throw error;
    }
}
...
///////COMMENTED OUT SECTION FOR RE CODED FUNCTIONS//////////////////////////////////////////
...
// Function to display book chapters in the list
/* function displayChaptersList(bookChapters) {
    // Clear existing chapters list
    $('#chaptersContainer').empty();

    // Iterate through chapters and append card for each chapter to the container
    for (let index = 0; index < bookChapters.book_chapters.length; index++) {
        const bookChapter = bookChapters.book_chapters[index];
...
// Extract bookIds from the array of objects
/*
const bookIds = bookData.map(book => book.book_id);

// Call the displayChaptersList function with the array of bookIds
displayChaptersList(bookIds);

// Initialize the execution flag
displayChaptersList.isExecuting = false;
*/

// Function to display chapters list in the UI
/* async function displayChaptersList(bookIds) {
    try {
        // Validate and sanitize bookIds
        const validatedBookIds = bookIds.map(bookId => parseInt(bookId)).filter(id => !Number.isNaN(id) && id > 0);

        if (validatedBookIds.length > 0) {
            // Clear existing chapters list
            $('#chaptersContainer').empty();

            // Fetch all book data for each bookId
            for (const validatedBookId of validatedBookIds) {
                const bookData = await fetchAllBookData(validatedBookId);
                const bookChapters = bookData.book_chapters;
                ...
                for (const bookChapter of bookChapters) {
                    ...
                    var chapterCard = $(
                        '<div class="card">' +
                            '<div class="card-header">' +
                                '<a class="btn btn-link chapter-link" data-bs-toggle="collapse" href="#chapterCollapse' +
                                bookChapter.chapter_id +
                                '" data-chapter-id="' +
                                bookChapter.chapter_id +
                                '">' +
                                'Chapter: ' +
                                ...
                                '</a>' +
                                '<button class="btn btn-primary" onclick="openEditChapterModal(' +
                                bookChapter.chapter_id +
                                ')">Edit Chapter</button>' +
                                '<button class="btn btn-danger" onclick="deleteChapter(' +
                                bookChapter.chapter_id +
                                ')">Delete Chapter</button>' +
                                '<button class="btn btn-success" id="addNewPageBtn" onclick="openNewPageModal(' +
                                bookChapter.chapter_id +
                                ');">Add Page</button>' +
                            '</div>' +
                            '<div id="chapterCollapse' +
                            bookChapter.chapter_id +
                            '" class="collapse" data-parent="#chaptersContainer">' +
                                '<div class="card-body">' +
                                    // List of pages and choices for Chapter
                                    '<ul id="pagesAndChoicesList' +
                                    bookChapter.chapter_id +
                                    '" class="list-group">' +
                                    '</ul>' +
                                '</div>' +
            ...
            $(document).off('click', '.chapter-link');

            // Event listener for collapse event
            $(document).on('click', '.chapter-link', async function () {
                var chapterId = $(this).data('chapter-id');

                // Ensure that both validatedBookId and chapterId are defined
                if (validatedBookIds && chapterId) {
                    try {
                        // Fetch and display pages for the current chapter using the new function
                        const pages = await fetchPageDetails(validatedBookIds[0], chapterId);
                        displayPagesInAccordion(chapterId, pages);
                    } catch (error) {
                        console.error('Error fetching pages:', error);
                    }
                } else {
                    console.error('validatedBookId or chapterId is not defined.');
                }
            });
        } else {
            console.error('Invalid book IDs');
        }
    } catch (error) {
...
/* function fetchSelectedBookChapters(bookId) {
    return new Promise((resolve, reject) => {
        console.log('Fetching book chapters for book ID from fetchSelectedBookChapters:', bookId);
        ...
        $.ajax({
            ...
            success: function (bookChapters) {
                console.log('AJAX Success. Received book chapters:', bookChapters);
...
fetchSelectedBookChapters(bookId)
 ...
 // Function to display chapters in the list
  ...
  function displayChaptersList(bookData) {
    // Clear existing chapters list
    $('#chaptersContainer').empty();
    ...
    // Display book details and preferences (you can customize this part)
    $('#bookDetails').text(`Book: ${bookDetails.title}, Author: ${bookDetails.author}`);
    $('#bookPreferences').text(`Genre: ${bookPreferences.genre}, Target Age: ${bookPreferences.target_age}`);
    ...
    $('.chapter-link').on('click', async function () {
        ...
        // Ensure that both selectedBookId and chapterId are defined
        if (bookDetails && chapterId) {
            try {
                // Find the corresponding pages for the current chapter in the combined result
                const pagesAndChoices = bookChapters.find(chapter => chapter.chapter_id === chapterId);
                displayPagesAndChoicesInAccordion(chapterId, pagesAndChoices);
            } catch (error) {
                console.error('Error fetching pages and choices:', error);
            }
        } else {
            console.error('Book details or chapterId is not defined.');
        }
    });
}
*/
...
// Function to open the modal for adding choices
function openChoiceModal(pageId, chapterId) {
    // Clear any previous input and reset the modal
    $('#newChoiceInputModal').val('');
    $('#saveChoiceModalBtn').off('click');

    // Attach a click event to the save button in the modal
    $('#saveChoiceModalBtn').on('click', function () {
        // Get the choice text from the modal input
        var choiceText = $('#newChoiceInputModal').val();
        ...
        if (choiceText.trim() !== '') {
            ...
            // Create a new page and save the choice
            createNewPageAndSaveChoice(pageId, chapterId, choiceText);
        } else {
            // Display an error message if the choice text is empty
...
function createNewPageAndSaveChoice(pageId, chapterId, choiceText) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Obtain Page's ID
            var newPageId = response.page_id;
        ...
        error: function (xhr, status, error) {
            console.error('Error creating new page:', status, error);
...
function saveChoice(pageId, chapterId, choiceText, targetPage) {
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log('New choice saved successfully:', response);

            // Display success alert for saving a new choice
            alert('New choice saved successfully!');
        },
        error: function (xhr, status, error) {
            console.error('Error saving new choice:', status, error);
            // Display error alert for saving a new choice
...
function displayChoices(choices, pageId, chapterId, amountOfChoices) {
    ...
    if (choices && Object.keys(choices).length > 0) {
        ...
        var addChoiceButton = '<button class="btn btn-success" onclick="openChoiceModal(' + pageId + ', ' + chapterId + ', ' + amountOfChoices + ')">Add Choice</button>';
        choicesList.append('<li class="list-group-item">' + addChoiceButton + '</li>');
    }
    console.log('Displaying choices for page ' + pageId + ':', choices);
    console.log('Displaying choices length:', Object.keys(choices).length);
...
// Functions to add and choices for the new page
...
function addChoice(pageId, chapterId, amountOfChoices) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            saveChoice(pageId, chapterId, $('#newChoiceInput' + pageId), newPageId);
        },
        error: function (xhr, status, error) {
            console.error('Error creating new page:', status, error);
            // Display error alert for creating a new page
...
/* console.log('Calling fetchSelectedBookChapters with selectedBookId:', selectedBookId);
fetchSelectedBookChapters(selectedBookId)
    .then(function (bookChapters) {
        console.log('Received book chapters:', bookChapters);
        displayChaptersList(bookChapters);
    })
    .catch(function (error) {
        console.error('Error fetching book chapters fetchSelectedBookChapters and handle the response:', error);
    });
...
<!-- Add this modal markup in your HTML -->
<div class="modal fade" id="addChoiceModal" tabindex="-1" aria-labelledby="addChoiceModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <label for="newChoiceInputModal" class="form-label">Enter choice text:</label>
                <input type="text" class="form-control" id="newChoiceInputModal">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-success" id="saveChoiceModalBtn">Save</button>
            </div>
        </div>
    </div>
</div>
