...
// Function to display choices dynamically
function displayChoices(choices, pageId, chapterId) {
    console.log('Displaying choices for page ' + pageId + ':', choices);

    var choicesList = $('#choicesList' + pageId);
    choicesList.empty();

    // Variable to track whether "Edit Choice" button has been added
    ...
    if (choices && Object.keys(choices).length > 0) {
        ...
        if (choices['choice_text'] !== undefined && choices['choice_text'] !== null) {
            ...
            // Create and append the list item with a radio button for choice_text
            var choiceItem = '<div>' +
                '<input type="radio" name="choiceRadio' + pageId + '" value="1">' + // Fixed value to 1
                choiceText;
        ...
        for (var i = 1; i <= 4; i++) {
            ...
            if (choices['choice_text_' + i] !== undefined && choices['choice_text_' + i] !== null) {
                ...
                // Create and append the list item with a radio button for each choice text
                var choiceItem = '<div>' +
                    '<input type="radio" name="choiceRadio' + pageId + '" value="' + (i + 1) + '">' + // Fixed value to (i + 1)
                    choiceText;
    ...
    var editChoiceButton = '<button class="btn btn-primary" onclick="openEditChoiceModal(' + pageId + ', ' + getSelectedChoice(pageId) + ', \'' + choices['choice_text'] + '\', \'' + choices['target_page'] + '\')">Edit Choice</button>';
    choicesList.append('<div>' + editChoiceButton + addChoiceButton + '</div>');
