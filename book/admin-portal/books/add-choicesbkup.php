...
if ($bookId !== null && $chapterId !== null && $pageId !== null && $targetPage !== null) {
    try {
        // Check if there's an existing entry with a NULL or default choice text for the same page ID
        $existingEntrySql = "SELECT * FROM choices WHERE book_id = ? AND chapter_id = ? AND page_id = ?";
        $stmtExistingEntry = $pdo->prepare($existingEntrySql);
        $stmtExistingEntry->execute([$bookId, $chapterId, $pageId]);
        ...
        if (count($existingEntries) > 0) {
            ...
            if ($firstNullOrDefaultColumn !== null) {
                ...
                $updateSql = "UPDATE choices SET $firstNullOrDefaultColumn = ?, target_page = ? WHERE choice_id = ?";
                $stmtUpdate = $pdo->prepare($updateSql);
                $stmtUpdate->execute([$choiceText, $targetPage, $existingEntries[0]['choice_id']]);
            } else {
