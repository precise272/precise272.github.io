...
function displayPagesInAccordion(chapterId, pages) {
    ...
    if (pages && pages.length > 0) {
        ...
        $.each(pages, function (index, page) {
            ...
            loadChoicesForPage(page.book_id, chapterId, page.page_id, displayChoices);
        });
    } else {
...
// Function to save the new page
function saveNewPage() {
    var newPageNumber = $('#newPageNumber').val();
    var newPageContent = $('#newPageContent').val();

    // Check if required fields are filled
    if (!newPageNumber || !newPageContent) {
        //console.error('Please fill in all required fields.');
        return;
    }
if (newPageNumber && newPageContent) {
    // Perform an AJAX request to save the new page to the database
    $.ajax({
        ...
        success: function (response) {
            if (response && response.message) {
                ...
                // Check if the page number was incremented
                if (response.page_number) {
                    // Update the page number input field
                    $('#newPageNumber').val(response.page_number);
                }

                // Update the pages list for the chapter on success
                fetchSelectedBookPages(selectedBookId, selectedChapterId)
                    .then(pages => {
                        // Display the updated pages
                        displayPagesInAccordion(selectedChapterId, pages);

                        // Add choices for the new page
                        //saveNewPageChoices(selectedChapterId, response.page_id);
                    })
                    .catch(error => {
                        console.error('Error fetching pages:', error);
                    });
                ...
                console.error('An unknown error occurred while saving the new page.');
            }
        },
        error: function (error) {
...
function loadChoicesForPage(bookId, chapterId, pageId, callback) {
    // Send an AJAX request to fetch choices for the current page
    $.ajax({
        ...
        success: function (response) {
            ...
            amountOfChoices = response.choices.length;

            // Call the callback function with the fetched choices, pageId, and chapterId
            callback(response.choices, pageId, chapterId);
        },
        error: function (error) {
...
// Function to load and display pages for a specific choice
function loadPagesForChoice(bookId, chapterId, choiceTargetPage, displayFunction, pageId, choiceNumber) {
    console.log('Loading pages for choice:', {
        bookId,
        chapterId,
        choiceTargetPage,
        pageId,
    ...
    $.ajax({
        url: '/book/admin-portal/books/load-pages-for-choice.php',
        method: 'POST',
        data: {
            book_id: bookId,
            chapter_id: chapterId,
            choice_target_page: choiceTargetPage
        },
        dataType: 'json',
        success: function (response) {
            console.log('Pages loaded successfully for choice:', response);
            if (response && response.pages) {
                // Call the display function to render pages for the choice
                displayFunction(response.pages, pageId, choiceNumber);
            } else {
                console.error('Error loading pages for choice. Response:', response);
            }
        },
        error: function (error) {
            console.error('AJAX error loading pages for choice:', error);
        }
    });
}
...
// Function to add a new choice for a page
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.error) {
                    ...
                    loadChoicesForPage(selectedBookId, selectedChapterId, pageId, displayChoices);
...
function refreshChoicesList(pageId, chapterId, amountOfChoices) {
    ...
    // It should reload the choices list for the specified page and chapter
    loadChoicesForPage(pageId, chapterId, displayChoices);
}
...
function openNewPageModal(chapterId) {
